    ...
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope('conv-maxpool-%s' % filter_size):
                ...
                # every filter learns a type of representation of word
        ...
        inputs = [tf.squeeze(input_, [1]) for input_ in
                  tf.split(pooled_concat, num_or_size_splits=reduced, axis=1)]
        # now inputs shape is : reduced * batch_size * num_filters
        ...
        # now output shape is : batch_size * hidden_unit
        with tf.name_scope('output'):
            ...
            self.probabilities = tf.nn.softmax(self.scores, dim=1, name='probabilities')
