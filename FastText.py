...
def show_chars_distribution():
    counter = {name: defaultdict(int) for name in author_lst}
    for (text, author) in zip(train_df['text'], train_df['author']):
        text = text.replace(' ', '')
        for c in text:
            counter[author][c] += 1
    ...
    for v in counter.values():
        chars |= v.keys()  # union operation of set
